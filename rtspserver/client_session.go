...
func (s *RTSPClientSession) handleCommandSetup(urlPreSuffix, urlSuffix, reqStr string) {
	...
	// === will setup the media transport
...
func (s *RTSPClientSession) handleCommandPlay(subsession livemedia.IServerMediaSubsession, fullRequestStr string) {
	...
	// === start the stream
	rtpSeqNum, rtpTimestamp := s.streamStates.subsession.StartStream(s.sessionID,
		s.streamStates.streamToken, s.noteLiveness,
		s.connection.handleAlternativeRequestByte)
