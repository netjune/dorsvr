...
var _log = ww.NewLogger("rtspserver", ww.Log_Level_DEBUG)
...
func (c *RTSPClientConnection) incomingRequestHandler() {
	...
	rbuf := make([]byte, 0, rtspBufferSize)

	buf := make([]byte, 2000)
	for {
		cnt, err := c.socket.Read(buf)
		if err != nil {
			_log.DBG("read error! %s\n", err.Error())
			break
		}

		rbuf = append(rbuf, buf[:cnt]...)

		for {
			pos := bytes.Index(rbuf, []byte("\r\n\r\n"))
			if pos < 0 {
				break
			}

			pos += 4
			err = c.handleRequestBytes(rbuf, pos)
			if err != nil {
